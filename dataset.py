...
class SelectionDataset(Dataset):
    def __init__(self, file_path, context_transform, response_transform, concat_transform, sample_cnt=None, mode='poly'):
        ...
        with open(file_path, encoding='utf-8') as f:
            ...
            # Here, is actually the per line in 'train.txt' being iterated
            for line in f:
                split = line.strip('\n').split('\t')
                lbl, context, response = int(split[0]), split[1:-1], split[-1]

                # Somehow handling positive response here
                ...
                # adding negative responses in to the neg_responses
                else:
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'cross':
            ...
            ### HERE IS THE ACTUAL USE OF CONTEXT, RESPONSE TRANSFORM
